...
      }
	...
	<?php 
			$query_yes= mysqli_query($connection,"select count(*) as score from verification_ir where score=1 and project='$proj'");
			$query_half= mysqli_query($connection,"select count(*) as score from verification_ir where score=0.5 and project='$proj'");
			$query_some= mysqli_query($connection,"select count(*) as score from verification_ir where score=0.2 and project='$proj'" );
			$query_no= mysqli_query($connection,"select count(*) as score from verification_ir where score=0 and project='$proj'");
	...
	<?php 
			$query_yes= mysqli_query($connection,"select count(*) as score from verification_ir where score=1 and project='$proj'");
			$query_no= mysqli_query($connection,"select count(*) as score from verification_ir where score<1 and project='$proj'");
...
<div class="containers">
  <ul class="nav nav-pills nav-justified">
    ...
    <li class="dropdown active"><a class="dropdown-toggle" data-toggle="dropdown" href="#">Verification<span class="caret"></span></a>
      <ul class="dropdown-menu">
        <li><a href="/vsamm1/dashboard/verification/verification_dr.php">Design Review</a></li>
          <li class="divider"></li>
        <li><a data-toggle='pill' href="/vsamm1/dashboard/verification/verification_ir.php">Implementation Review</a></li>
          <li class="divider"></li>
...
<br><br>
	...
	<div class="h3">Verification</div>
		...
		<div class="col-md-4 pad card">
		<div class="h3">Implementation Review</div>
        <div id="columnchart_materialS" style="width: 100%; height: 360px;"></div>
        <h4 style="color:black;"><b>Total Score</b>:<span><?php echo number_format((float)$ir->score, 1, '.', '');?>/&nbsp;<?php echo $ircount->total; ?></span></h4>
        </div>
...
$table  = mysqli_query($connection ,"SELECT * FROM verification_ir where project='$proj' ORDER BY id ASC LIMIT $from_record_num, $records_per_page");
		if($table){
          while($row  = mysqli_fetch_array($table)){ ?>
              <tr id="<?php echo $row['id']; ?>">
                <td data-target="vsammid"><?php echo "VER-IR ".$row['id']; ?></td>
...
// count total number of rows
		$query = "SELECT COUNT(*) as total_rows FROM verification_ir where project='$proj' ";
      ...
      // now create event to get data from fields and update in database 
       ...
       $('#save').click(function(){
		...
		var files = $('#file')[0].files[0];
		   ...
		   fd.append('tbl','verification_ir');
		   fd.append('secprac',"Implementation Review");
